...
public class FifoScheduler extends
  ...
  private int assignContainer(FiCaSchedulerNode node, FifoAppAttempt application,
    ...
    if(capability.getGPUs() > 0) {
        assignedContainers = Math.min(assignedContainers, node.getUnallocatedResource().getGPUs() / capability.getGPUs());
    }
    ...
    if (assignedContainers > 0) {
      for (int i=0; i < assignedContainers; ++i) {
        ...
        if(capability.getGPUs() > 0) {
          // Allocate!
          LOG.info("GPU allocation request: " + capability.toString() + " from availability: " + node.getUnallocatedResource().toString());
          long allocatedGPU = Resources.allocateGPUs(capability, node.getUnallocatedResource());
          capability.setGPUAttribute(allocatedGPU);
          node.getUnallocatedResource().setGPUAttribute(node.getUnallocatedResource().getGPUAttribute() | allocatedGPU);
        }
